"""
Extract features from YOLO v20 and evaluate competition score
"""
...
if __name__ == '__main__':
    print("=" * 70)
    print("YOLO v20 Full Pipeline: Extract Features + Train + Evaluate")
    print("=" * 70)

    camera_lat, camera_lon = 14.305029, 101.173010

    # 1. Extract YOLO v20 features
    print("\n📦 Loading YOLO v20...")
    model = YOLO('runs/detect/drone_detect_v20_real_labels/weights/best.pt')
    ...
    for idx, row in df.iterrows():
        ...
        if len(results) == 0 or results[0].boxes is None or len(results[0].boxes) == 0:
            ...
            yolo_features.append({
                ...
                'bbox_x': float(x_center), 'bbox_y': float(y_center),
                'bbox_w': float(width), 'bbox_h': float(height),
    ...
    feature_columns = [
        ...
        'distance_x_conf', 'distance_x_area'
    ]
    ...
    # 4. Evaluate
    print("\n📊 Evaluating...")
    df_val = result_df.iloc[X_val[:, 0].astype(bool)]  # Hack to get validation indices
    ...
    print("\n" + "=" * 70)
    print("📊 YOLO v20 Competition Score")
    print("=" * 70)
